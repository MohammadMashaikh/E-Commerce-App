...
<div class="mx-10 mt-14 flex flex-col md:flex-row items-center justify-between gap-4">
    <!-- Search bar -->
    <form action="{{ route('admin.users.list') }}" method="GET" class="flex w-full md:w-1/2">
    ...
    <!-- Add user button -->
    @can('manage-users', auth('admin')->user())
...
<div class="bg-white shadow-lg rounded-2xl overflow-hidden m-10">
    <h2 class="text-lg font-semibold p-6 border-b">Recent Users</h2>
        ...
        <table class="w-full text-left min-w-[600px]">
            ...
            <tbody class="text-gray-700">
                ...
                <tr class="border-b hover:bg-gray-50">
                    <td class="p-4">
                        @if($user->hasMedia('admin-user-image'))
                            <img src="{{ $user->getFirstMediaUrl('admin-user-image') }}" 
                                alt="{{ $user->name }}" 
                    ...
                    <td class="p-4">{{ $user->created_at->format('h:i A') }}</td>
                    ...
                    <form action="{{ route('admin.users.delete', $user->id) }}" method="POST" onsubmit="confirmation(event)">
                        @csrf
                        @method('DELETE')
                ...
                @empty
                <tr><td class="p-4 text-center text-gray-500" colspan="6">No Available users Yet</td></tr>
